...
def main():
    ...
    workspace_repo_file = root / "core" / "data" / "src" / "main" / "kotlin" / "sh" / "haven" / "core" / "data" / "preferences" / "WorkspaceRepo.kt"
    repo_sheet_file = root / "feature" / "terminal" / "src" / "main" / "kotlin" / "sh" / "haven" / "feature" / "terminal" / "arh" / "WorkspaceRepoSelectorSheet.kt"
    journaler_file = root / "feature" / "terminal" / "src" / "main" / "kotlin" / "sh" / "haven" / "feature" / "terminal" / "arh" / "SessionJournaler.kt"
    ...
    if not adr2_file.exists():
        print(f"[FAIL] Missing ADR-002 in docs/adr")
        sys.exit(1)
    if "ADR-002" not in adr2_file.read_text(encoding="utf-8"):
        print(f"[FAIL] ADR-002 not found in ADR-002 file")
        sys.exit(1)
    ...
    if not workspace_repo_file.exists():
        print(f"[FAIL] Missing WorkspaceRepo.kt")
        sys.exit(1)
    ...
    if "data class WorkspaceRepo" not in repo_code or "DEFAULT_REPOS" not in repo_code:
        print(f"[FAIL] WorkspaceRepo.kt missing required model definitions")
        sys.exit(1)
    print("[PASS] WorkspaceRepo data model & serialization verified.")

    pref_code = pref_file.read_text(encoding="utf-8")
    for key in ["agentMacroBarEnabled", "agentMacros", "agentCodeExtractorEnabled", "agentHyperlinkRoutingEnabled", "agentSavedPrompts", "agentSavedWorkspaces", "mcpAutoApproveEnabled", "mcpCustomPort", "mcpCustomHost"]:
    ...
    for ext_file in [parser_file, macro_bar_file, sheet_file, prompt_sheet_file, exporter_file, repo_sheet_file, journaler_file]:
        if not ext_file.exists():
            print(f"[FAIL] Missing terminal ARH extension file: {ext_file.name}")
            sys.exit(1)
    print("[PASS] CodeBlockParser, AgentMacroBar, CodeExtractionSheet, PromptBookSheet, TerminalMarkdownExporter, WorkspaceRepoSelectorSheet, and SessionJournaler verified.")
